...
test("Validation should fail for ticket number", () => {
  expect(validate("feat(Mattermost node): add new resource rollout-1234"))
    .toHaveLength(1)
    .toContain(ERRORS.TICKET_NUMBER_PRESENT);

  expect(validate("rollout-1234 feat(Mattermost node): add new resource"))
    .toHaveLength(1)
    .toContain(ERRORS.TICKET_NUMBER_PRESENT);

  expect(validate("feat(Mattermost node) rollout-1234: add new resource"))
    .toHaveLength(1)
    .toContain(ERRORS.TICKET_NUMBER_PRESENT);
});
